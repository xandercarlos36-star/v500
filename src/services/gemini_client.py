        ...
        try:
            ...
            try:
                if hasattr(response, 'text') and response.text:
                    ...
                    raise Exception("Não foi possível extrair texto da resposta do Gemini")

                if not response_text:
                    raise Exception("Resposta vazia do Gemini")
            except Exception as extract_error:
                logger.error(f"❌ Erro ao extrair texto do Gemini: {extract_error}")
                raise Exception(f"Falha ao extrair resposta do Gemini: {extract_error}")
