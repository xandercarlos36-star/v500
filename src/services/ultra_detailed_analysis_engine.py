...
from services.competitor_analysis_engine import competitor_analysis_engine
from services.sales_funnel_optimizer import sales_funnel_optimizer
from services.strategic_positioning_engine import strategic_positioning_engine
from services.strategic_action_planner import strategic_action_planner
from services.strategic_keywords_analyzer import strategic_keywords_analyzer
...
class UltraDetailedAnalysisEngine:
    ...
    def generate_gigantic_analysis(
        ...
        try:
            ...
            if progress_callback:
                progress_callback(15, "🏆 Analisando concorrência...")

            # 8. ANÁLISE DE CONCORRÊNCIA - OBRIGATÓRIA
            competitor_analysis = self._execute_competitor_analysis(data, avatar_data)

            if progress_callback:
                progress_callback(17, "📊 Otimizando funil de vendas...")

            # 9. FUNIL DE VENDAS OTIMIZADO - OBRIGATÓRIO
            sales_funnel = self._execute_sales_funnel_optimization(avatar_data, data)

            if progress_callback:
                progress_callback(19, "🎯 Definindo posicionamento estratégico...")

            # 10. POSICIONAMENTO ESTRATÉGICO - OBRIGATÓRIO
            strategic_positioning = self._execute_strategic_positioning(avatar_data, data)

            if progress_callback:
                progress_callback(21, "📋 Criando plano de ação...")

            # 11. PLANO DE AÇÃO ESTRATÉGICO - OBRIGATÓRIO
            action_plan = self._execute_strategic_action_plan(data)

            if progress_callback:
                progress_callback(23, "🔤 Analisando palavras-chave...")

            # 12. ANÁLISE DE PALAVRAS-CHAVE - OBRIGATÓRIA
            keywords_analysis = self._execute_keywords_analysis(avatar_data, research_data, data)
            ...
            gigantic_analysis = {
                ...
                "analise_concorrencia": competitor_analysis,
                "funil_vendas_otimizado": sales_funnel,
                "posicionamento_estrategico": strategic_positioning,
                "plano_acao_estrategico": action_plan,
                "palavras_chave_estrategicas": keywords_analysis,
    ...
    def _execute_avatar_analysis(self, data: Dict[str, Any], research_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not json_text:
            try:
                ...
                # Verifica se research_data é dict e tem extracted_content
                extracted_count = 0
                ...
                avatar_data["metadata_avatar"] = {
                    "fontes_utilizadas": extracted_count,
