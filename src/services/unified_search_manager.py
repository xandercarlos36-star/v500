...
class UnifiedSearchManager:
    ...
    def unified_search(
        ...
        try:
            ...
            # 1. Busca prioritária otimizada: Alibaba WebSailor > Exa AI > Google CSE
            search_providers = [
                {'name': 'websailor', 'weight': 0.4},  # 40% dos resultados
                {'name': 'exa', 'weight': 0.35},       # 35% dos resultados  
                {'name': 'google', 'weight': 0.25}     # 25% dos resultados
            ]

            for provider_config in search_providers:
                provider = provider_config['name']
                provider_max_results = int(max_results * provider_config['weight'])
                
                try:
                    if provider == 'websailor':
                        results = self._search_with_websailor_optimized(query, provider_max_results, context)
                    elif provider == 'exa':
                        results = self._search_with_exa_optimized(query, provider_max_results, context)
                    elif provider == 'google':
                        results = self._search_with_google_optimized(query, provider_max_results, context)
                    else:
                        continue
                    ...
                    if results and len(results) > 0:
                        ...
                        logger.info(f"✅ {provider}: {len(results)} resultados (peso: {provider_config['weight']*100:.0f}%)")
    ...
    def _search_with_websailor_optimized(self, query: str, max_results: int, context: Dict[str, Any] = None) -> List[Dict]:
        """Busca otimizada com Alibaba WebSailor (PRIORIDADE 1)"""
        try:
            from services.alibaba_websailor import AlibabaWebSailorAgent
            
            websailor_agent = AlibabaWebSailorAgent()
            
            # Executa busca profunda otimizada
            websailor_results = websailor_agent.navigate_and_research_deep(
                query=self._enhance_query_for_brazil(query),
                data=context or {},
                max_pages=max_results,
                depth_levels=3,  # Busca mais profunda
                session_id=context.get('session_id') if context else None
            )
            
            if websailor_results and websailor_results.get('status') == 'success':
                processed_results = websailor_results.get('processed_results', [])
                
                # Converte para formato padrão
                standardized_results = []
                for item in processed_results[:max_results]:
                    standardized_results.append({
                        ...
                        'content_quality': item.get('content_quality', 'high'),
                        'extraction_depth': item.get('extraction_depth', 1)
                    })
            ...
            logger.error(f"❌ Erro WebSailor otimizado: {e}")
            return []

    def _search_with_exa_optimized(self, query: str, max_results: int, context: Dict[str, Any] = None) -> List[Dict]:
        """Busca otimizada com Exa AI (PRIORIDADE 2)"""
        try:
            from services.exa_client import exa_client
            
            # Configura busca com parâmetros otimizados
            enhanced_query = self._enhance_query_for_brazil(query)
            
            # Domínios brasileiros preferenciais para Exa
            include_domains = [
                "g1.globo.com", "exame.com", "valor.globo.com", "estadao.com.br",
                "folha.uol.com.br", "infomoney.com.br", "startse.com",
                "revistapegn.globo.com", "epocanegocios.globo.com"
            ]
            ...
            logger.error(f"❌ Erro Exa otimizado: {e}")
            return []

    def _search_with_google_optimized(self, query: str, max_results: int, context: Dict[str, Any] = None) -> List[Dict]:
        """Busca otimizada com Google CSE (PRIORIDADE 3)"""
        try:
            from services.production_search_manager import production_search_manager
            
            # Usa o manager de produção existente
            google_results = production_search_manager.search_with_google(
                self._enhance_query_for_brazil(query), 
                max_results
            )
            ...
            logger.error(f"❌ Erro Google otimizado: {e}")
            return []
